...
- **Nightly agent** ([`nightly-agent.yml`](.github/workflows/nightly-agent.yml)) plans-then-act on the lowest-ID open backlog task under a 10-call budget; skips tasks whose implement payload exceeds 100 KiB and tries the next eligible task in the same run; raises a PR on success or a draft PR on implement/test failure.
