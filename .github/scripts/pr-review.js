...
// Matches FIX_MAX_TOKENS: reviewing a whole diff is harder than fixing one file,
// so it must not get a smaller budget. See the note at the call site on why this
// number buys less visible output than it looks.
...
Order the array most severe first — all Blockers, then all Majors, then all Minors. If the response is cut short, the findings that survive must be the most important ones.
...
async function requestReview(skillContent, diff, apiKey) {
  ...
  const response = await fetchWithRetry(
    'https://api.anthropic.com/v1/messages',
    {
      ...
      body: JSON.stringify({
        model: REVIEW_MODEL,
        // Claude 5 models run adaptive thinking when `thinking` is omitted, and
        // max_tokens caps thinking plus visible output together — so the budget
        // for the JSON array is well under the nominal number.
  ...
  // A truncated array cannot be parsed, so the findings are lost either way.
  // Log the raw text before discarding it — without this the review is
  // unrecoverable, and the caller degrades to an advisory draft rather than
  // exiting non-zero and posting nothing at all.
  if (truncated) {
    warn(
      `Review truncated at max_tokens (${REVIEW_MAX_TOKENS}) — findings could not be parsed, posting an advisory comment and marking the PR draft`,
    );
    // eslint-disable-next-line no-console
    console.error('[pr-review] Raw truncated API response text:');
    // eslint-disable-next-line no-console
    console.error(typeof text === 'string' ? text : '(no text content block)');
...
// A truncated review yields no parseable findings at all, so this never reports
// a clean diff — saying "no findings" when the response was cut short would be
// an affirmatively false statement on the PR.
function formatTruncatedComment({ afterFixes = false } = {}) {
  const context = afterFixes
    ? 'The re-review after the automated fix commit was cut short, so the fixes on this branch are **unverified**.'
    : 'The review response was cut short before any finding could be read.';

  return `${BOT_MARKER}

## Automated PR review — needs human review

Reviewed against the code-reviewer skill. **The review did not complete.**

**Summary:** No findings could be read — the model hit the ${REVIEW_MAX_TOKENS}-token response limit. ${context}

**This is not a clean review.** An unknown number of findings of unknown severity were lost, so nothing here should be read as approval.

**Status:** PR marked as **draft**. Please review manually, then remove draft status when ready.

The raw truncated response is in the workflow logs. If this recurs, the diff is likely large enough to need splitting across smaller PRs.
...
async function runFixLoop({
  ...
  for (let attempt = 1; attempt <= MAX_FIX_ATTEMPTS; attempt += 1) {
    ...
    const { text: raw, truncated: reviewTruncated } = await requestReview(
      skillContent,
      diff,
      apiKey,
    );
    ...
    if (reviewTruncated) {
      ...
      return {
        outcome: 'truncated',
        comment: formatTruncatedComment({ afterFixes: true }),
      };
    }
...
async function main() {
  ...
  const { text: rawFindings, truncated } = await requestReview(
    skillContent,
    diff,
    apiKey,
  );

  // Nothing was parseable, so there is nothing for the fix loop to act on.
  // Post the truncation notice and mark draft rather than exiting non-zero and
  // leaving the PR with no review at all.
  if (truncated) {
    if (isGitHubActions) {
      await markPrDraft(owner, repo, prNumber, token);
    }
    ...
    log('PR review complete (truncated — no findings read, PR marked draft)');
    return;
  }
