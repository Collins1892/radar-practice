...
surfacing the failure rather than going silent.

The self-repair path degraded rather than crashed: two fix calls returned
`invalid_json`, and the agent fell back to re-implementing from the
...
The live task pickup landed on T17 — the same task the agent had failed
...
wrong direction initially — looking for a bad assertion rather than a
module that never loaded.

Lesson: a failure note recording the fact of failure but not its shape is
close to useless. "npm test (client) failed" and "0 tests collected in
...
single `it()` block — the exact check that would have caught the agent's
failure on attempt one. Four synchronous title assertions. Pending-promise
mocks on the detail and edit routes so `IncidentDetailView` could not
overwrite `document.title` after load.
...
fails safely and says why produces more usable material than one that
succeeds quietly — the reset, the backlog-only PR and the failure note
were the demo, not the workaround.
